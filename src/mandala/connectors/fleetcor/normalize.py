...
SOURCE = "mandala/connector/fleetcor"
...
def _truck_urn(vehicle_id: Any) -> str:
    return str(URN.truck(scope="fleetcor", id=str(vehicle_id)))


def normalize_transaction(transaction: dict[str, Any]) -> MandalaEvent:
    """Convert a FLEETCOR transaction into a mandala.truck.fueled event.

    Args:
        transaction: FLEETCOR API transaction response
...
def normalize_transactions(transactions: list[dict[str, Any]]) -> list[MandalaEvent]:
    """Convert multiple FLEETCOR transactions into MandalaEvent objects.

    Args:
        transactions: List of FLEETCOR API transaction responses
