"""Normalize WEX API payloads into MandalaEvent objects."""
...
def _truck_urn(vehicle_id: Any) -> str:
    return str(URN.truck(scope="wex", id=str(vehicle_id)))


def normalize_transaction(transaction: dict[str, Any]) -> MandalaEvent:
    """Convert a WEX transaction into a mandala.truck.fueled event.

    Args:
        transaction: WEX API transaction response
...
def normalize_transactions(transactions: list[dict[str, Any]]) -> list[MandalaEvent]:
    """Convert multiple WEX transactions into MandalaEvent objects.

    Args:
        transactions: List of WEX API transaction responses
